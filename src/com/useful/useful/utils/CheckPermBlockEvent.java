...
public class CheckPermBlockEvent extends BlockPlaceEvent {
    ...
    Player p = null;
	public CheckPermBlockEvent(Block placedBlock, Material mat, 
	...
	public CheckPermBlockEvent(Block placedBlock, int id, Byte rawData,  
			BlockState replacedBlockState, Block placedAgainst,
			ItemStack itemInHand, Player thePlayer, boolean canBuild, BlockState originalBlockData) {
		super(placedBlock, replacedBlockState, placedAgainst, itemInHand, thePlayer,
				canBuild);
...
public void confirm(Boolean worked){
	if(Builder.tests.containsKey(this.p.getName())){
	Builder.tests.put(this.p.getName(), worked);
	}
	return;
}
